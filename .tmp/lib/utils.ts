...
export const isInArray: (value: any, array: string|Array<any>) => boolean = (
  value,
  array
) => {
...
export const deepExtend: (destination: any, source: any) => any = function(
  destination,
  source
) {
...
export function createSvgImg(particle: Particle, tmp: ITmpParams): void {
  ...
  let coloredSvgXml: string = svgXml.replace(rgbHex, (m, r, g, b) => {
    ...
    if (particle.color.rgb) {
      let { _r, _g, _b } = particle.color.rgb;
