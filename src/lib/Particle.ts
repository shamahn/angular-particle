  ...
  private _checkOverlap(
    ...
    particles.array.forEach((particle: Particle) => {
      ...
      if (dist <= p1.radius + p2.radius) {
        ...
        p1.y = position
          ? position.y
          : Math.random() * this._canvasParams.height;
    ...
    switch (this.shape) {
      ...
      case 'polygon':
        this._drawShape(
          this._canvasParams.ctx,
          this.x -
            radius / (this._params.particles.shape.polygon.nb_sides / 3.5),
      ...
      case 'star':
        this._drawShape(
          this._canvasParams.ctx,
          this.x -
            (radius * 2) / (this._params.particles.shape.polygon.nb_sides / 4),
          this.y - radius / ((2 * 2.66) / 3.5),
          (radius * 2 * 2.66) /
            (this._params.particles.shape.polygon.nb_sides / 3),
          this._params.particles.shape.polygon.nb_sides,
          2
        );
        break;
      ...
      case 'image':
        let draw: (img_obj: any) => void = _img_obj => {
