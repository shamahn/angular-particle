...
export class ParticlesManager {
    ...
    this._params.particles.array.forEach((particle: Particle, i: number) => {
      ...
      if (this._params.particles.opacity.anim.enable) {
        if (particle.opacity_status === true) {
          ...
          if (
            particle.opacity <= this._params.particles.opacity.anim.opacity_min
          ) {
      ...
      if (
        this._params.particles.line_linked.enable ||
        this._params.particles.move.attract.enable
      ) {
        for (let j = i + 1; j < this._params.particles.array.length; j++) {
          ...
          if (this._params.particles.line_linked.enable) {
            this._interaction.linkParticles(
              particle,
              link,
              this._params,
              this._canvasParams
  ...
  public particlesDraw(): void {
    this._canvasParams.ctx.clearRect(
      0,
      0,
      this._canvasParams.width,
      this._canvasParams.height
    );
  ...
  private _bubbleParticle(particle: Particle) {
    if (
      this._params.interactivity.events.onhover.enable &&
      isInArray('bubble', this._params.interactivity.events.onhover.mode)
    ) {
      let dx_mouse: number =
        particle.x - this._params.interactivity.mouse.pos_x;
      let dy_mouse: number =
        particle.y - this._params.interactivity.mouse.pos_y;
      let dist_mouse: number = Math.sqrt(
        dx_mouse * dx_mouse + dy_mouse * dy_mouse
      );
      let ratio: number =
        1 - dist_mouse / this._params.interactivity.modes.bubble.distance;
      ...
      if (dist_mouse <= this._params.interactivity.modes.bubble.distance) {
        if (ratio >= 0 && this._params.interactivity.status === 'mousemove') {
          if (
            this._params.interactivity.modes.bubble.size !==
            ...
            if (
              ...
              let dif: number =
                particle.radius - this._params.interactivity.modes.bubble.size;
          ...
          if (
            ...
            if (
              ...
              let opacity: number =
                this._params.interactivity.modes.bubble.opacity * ratio;
              if (
      ...
      this._params.interactivity.events.onclick.enable &&
      isInArray('bubble', this._params.interactivity.events.onclick.mode)
    ) {
      if (this._tmpParams.bubble_clicking) {
        let dx_mouse: number =
          particle.x - this._params.interactivity.mouse.click_pos_x;
        let dy_mouse: number =
          particle.y - this._params.interactivity.mouse.click_pos_y;
        let dist_mouse: number = Math.sqrt(
          dx_mouse * dx_mouse + dy_mouse * dy_mouse
        );
        let time_spent: number =
          (new Date().getTime() - this._params.interactivity.mouse.click_time) /
          1000;
        ...
        let process: any = (
          ...
          if (bubble_param !== particles_param) {
            if (!this._tmpParams.bubble_duration_end) {
              if (
                dist_mouse <= this._params.interactivity.modes.bubble.distance
              ) {
                let obj: any;
                ...
                if (obj !== bubble_param) {
                  let value: any =
                    ...
                    (time_spent * (p_obj - bubble_param)) /
                      this._params.interactivity.modes.bubble.duration;
                  if (id === 'size') {
  ...
  private _repulseParticle(particle: Particle) {
    if (
      ...
      let repulseRadius: number = this._params.interactivity.modes.repulse
        .distance;
      let velocity = 100;
      ...
      if (!this._tmpParams.repulse_finish) {
        ...
        if (
          this._tmpParams.repulse_count === this._params.particles.array.length
        ) {
      ...
      if (this._tmpParams.repulse_clicking) {
        let repulseRadius: number = Math.pow(
          this._params.interactivity.modes.repulse.distance / 6,
          3
        );

        let dx: number =
          this._params.interactivity.mouse.click_pos_x - particle.x;
        let dy: number =
    ...
    if (
      ...
      if (dist_mouse <= interactivity.modes.grab.distance) {
        ...
        if (opacity_line > 0) {
          ...
          this._canvasParams.ctx.lineTo(
            interactivity.mouse.pos_x,
            interactivity.mouse.pos_y
          );
